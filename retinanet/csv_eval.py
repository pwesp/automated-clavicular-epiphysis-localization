...
def evaluate(
    ...
    for label in range(generator.num_classes()):
        ...
        for i in range(len(generator)):
            ...
            for d in detections:
                ...
                if annotations.shape[0] == 0:
                    ...
                    ious            = np.append(ious, 0.0)
                    continue

                # compute overlap of detection with all labelled annotations for the current class in this image
        ...
        # sort metrics by score in descending order to compute average precision
        indices         = np.argsort(-scores)
        false_positives = false_positives[indices]
        true_positives  = true_positives[indices]
        ious            = ious[indices]

        # compute cumulative sum of false positives and true positives in descending score order
        false_positives = np.cumsum(false_positives)
        true_positives  = np.cumsum(true_positives)

        # compute recall and precision in descending score order
